...
/**
 * Loads a pet used by biometric routes and enforces either direct ownership or,
...
export async function loadAuthorizedPet(
  ...
  petId: string,
  options: { allowNgo?: boolean; notFoundKey?: string } = {}
): Promise<AuthorizedPet> {
  const authContext = requireAuthContext(event);
  const allowNgo = options.allowNgo !== false;
  const notFoundKey = options.notFoundKey || 'petBiometric.errors.petNotFound';
